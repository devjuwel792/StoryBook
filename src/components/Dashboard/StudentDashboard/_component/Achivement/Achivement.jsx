...
const TimelineItem = ({
  ...
  return (
    <div className="relative flex items-center gap-4 mb-8 last:mb-0">
      {/* Timeline node */}
      <div
        className={`relative z-10 w-14 h-14   rounded-full flex items-center justify-center text-white font-semibold text-sm ${getStatusColor()} ${
          highlight ? "ring-2 ring-yellow-300 ring-offset-2" : ""
        }`}
      >
        {getStatusIcon()}
      </div>
      ...
      <div
        className={`flex-1 ${
          highlight ? "bg-yellow-50 border border-yellow-100" : "bg-gray-50"
        } p-4 rounded-xl`}
      >
